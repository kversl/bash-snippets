...
# Zusätzlihce Funktion:
# Permanent reservierte IP Adresse 192.168.253.117
#
# TODO
# Fälle: 
# - Anfangsbedingung: Recent List is leer
# - current IP größer als größte recent IP 
...
function recent_now_compare_lists(){
  ...
  for thisrline in "${recentlist[@]}"; do
    ...
    while [[ $inow -lt $lnow ]] ; do
      ...
      echo -ne "rline:$(ipdecimal_to_ipstring $recent_ip) ... " # debug
      
      if [[ $recent_ip -gt $now_ip ]]; then
        # -    IP new in NOW >> OK message, insert NOW line
        thisnlineparts[2]=$curdate
        send_msg 5 "NEW IP: $(ipdecimal_to_ipstring $now_ip) \n      $(ipdecimal_to_ipstring $now_ip)   $now_mac  $now_rest" 
        ...
        echo "Recent > Now;  nowline:$(ipdecimal_to_ipstring $now_ip)" # debug
        ...
        echo "Recent < Now;  nowline:$(ipdecimal_to_ipstring $now_ip)  NOOP" # debug
        ...
        if [[ $recent_mac = $now_mac ]]; then
          ...
          send_msg 2 "CONFLICTING NEW IP: $(ipdecimal_to_ipstring $now_ip) \n      $(ipdecimal_to_ipstring $now_ip)   $now_mac   $now_rest\n      $recent_ip_str   $recent_mac   $recent_rest" 
